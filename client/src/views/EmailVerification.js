      ...
      .then((res) => {
        if (res.status == statusCode.HTTP_200_OK) {
          this.setState({
            ...
            status: true,
            message:
              "We have shared a verification link to your email. Please check your inbox for the same!",
          });
        } else {
          ...
          this.setState({
            status: false,
            loader: false,
            message: "Sending failed: " + res.response.data.detail,
          });
        }
      })
      .catch((err) => {
      ...
      verifyEmail(payload)
        .then((res) => {
          if (res.status == statusCode.HTTP_200_OK) {
            ...
            this.setState({
              status: false,
              showSendVefificationLink:
                res.response.data.detail ==
                "User email verification link has expired"
                  ? true
                  : false,
              loader: false,
      ...
      <div className="registerWrapper">
        ...
        <div className="verificationDetail">
          ...
          <div className="forgetPass align-items-center">
            <p
              className={`${this.state.loader ? "" : "titleCard"}  ${
                this.state.status ? "primary" : "text-danger"
              }`}
            >
              {this.state.loader ? (
            ...
            {this.state.status === true && (
              <p>
                ...
                <span id="seconds">7</span> seconds.
              </p>
            )}
            {this.state.showSendVefificationLink === true && (
              <CardText className="mb-2">
                Your verification link has expired. Click{" "}
                <a
